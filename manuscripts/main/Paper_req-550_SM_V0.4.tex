...
with $\beta_{ij}=\tau C_{ij}$, $\tau$ is the probability of infection
...
class $i$ with an individual of class $j$ per day. The model is
illustrated in Fig. \ref{fig:Diagram}, and the rest of the parameters
...
days (95\% CI: 0.8-3.0). Since a correction of these results by Ashcroft
et al. \cite{ashcroft_covid-19_2020} suggest they significantly underestimate
...
\begin{tabular}{|>{\centering}p{0.1\textwidth}|>{\centering}p{0.075\textwidth}|>{\centering}p{0.075\textwidth}|>{\centering}p{0.1\textwidth}|>{\centering}p{0.1\textwidth}|>{\centering}p{0.1\textwidth}|>{\centering}p{0.1\textwidth}|>{\centering}p{0.1\textwidth}|>{\centering}p{0.1\textwidth}|}
\hline 
...
adults in age group 2. In accordance with camp manager expectations
...
\textless 13 and 60\% of the remainder of the green zone to adults
13-50 without comorbidities. We also consider a baseline scenario
in which there is no green zone.
...
We considered the existence of a safety zone to isolate a certain
fraction $f_{\mathrm{S}}$ of the population, mostly those more vulnerable.
In practice, this involves dividing the camp in two areas, a ``green''
zone (denoted $\g$) for the vulnerable population and an ``orange''
zone ($\oran$) for the remaining population. These two populations
could interact via a buffer zone, under controlled conditions. Based
on our assumption that infectivity in the buffer zone is reduced by
80\%, $\hat{\tau}=0.2\tau$. Each individual in the green zone can
...
interaction with members of the classes staying in the same area compared
...
with $N_{\X}$ the total number of individuals in the area $X=\{\oran,\g\}$.
...
Carers are entirely drawn from adults (age 2) with no comorbidities,
...
Given these assumptions, the number of contacts that the healthy adult
(age 2) population class will have with the isolated population will
...
this probability is equal to one for the healthy adult population
...
time to recognize their symptoms and to decide that self-isolation
is needed. To model this, whenever simulating the isolation intervention,
the symptomatic compartment is split in two compartments: onset of
symptoms, $I_{i}^{\onset}$ and symptomatic, $I_{i}$. We assumed
the duration of $I_{i}^{\onset}$ follows a Gaussian distribution
with means 12, 24 or 48 hours on average. The duration for which an
...
\begin{figure}[H]
\includegraphics[width=1\textwidth]{figures/newFig/FigS2}\hspace{2mm}\caption{\label{fig:Suppl_DvsR} \textbf{Outcomes when all severe cases recover
...
\begin{figure}[H]
\centering{}\includegraphics[width=1\textwidth]{figures/newFig/FigS3}\hspace{2mm}\caption{\label{fig:Suppl_self} \textbf{Self-distancing.} CFR (left), and
fraction of the population that recovers (right) as a function of
proportion of contact reduced.}
\end{figure}
\medskip{}

\begin{figure}[H]
\centering{}\includegraphics[width=1\textwidth]{figures/newFig/FigS4}\hspace{2mm}\caption{\label{fig:Suppl_isolation} \textbf{Self-isolation.} CFR (left),
...
\begin{figure}[H]
\includegraphics[width=1\textwidth]{figures/newFig/FigS5}\hspace{2mm}\caption{\label{fig:Suppl_onset} \textbf{Time to self-isolation.} Probability
...
\begin{figure}[H]
\includegraphics[width=1\textwidth]{figures/newFig/FigS6}\hspace{2mm}\caption{\label{fig:Suppl_evacuation} \textbf{Evacuation.} Probability of
an outbreak (top left), fraction of the population dying (top middle),
time until peak symptomatic cases (top right), CFR (bottom left),
and fraction of the population that recovers (bottom middle), depending
on whether individuals requiring hospitalization are evacuated to
isolation centers, in tandem with no other interventions, and with
a safety zone with 2 contacts per week.}
\end{figure}

\medskip{}

\begin{figure}[H]
\includegraphics[width=1\textwidth]{figures/newFig/FigS7}\hspace{2mm}\caption{\label{fig:Suppl_Tcheck} \textbf{Health-checks in the buffer zone.}
...
middle), depending on whether health-checks are implemented in the
buffer zone with 10 and with 2 contacts per week.}
\end{figure}

\begin{figure}[H]
\centering{}\includegraphics[width=0.6\textwidth]{figures/newFig/Fig_agegroups}\hspace{2mm}\caption{\label{fig:Suppl_agegroups} \textbf{Effects of the safety zone on
...
are implemented (Mixed), compared to protection of age 3 in a safety
zone with 2 contacts per week (Safety zone). The fraction of deaths
in the safety zone for the older population is significantly lower
(Kruskal-Wallis test, p-val$<10^{-15}$).}
\end{figure}
\medskip{}

\begin{figure}[H]
\centering{}\includegraphics[width=1\textwidth]{figures/newFig/FigS8}\hspace{2mm}\caption{\label{fig:Suppl_safety} \textbf{Number of contacts in the buffer
zone.} CFR (left), and fraction of the population that recovers (right)
as a function of the number of contacts per individual in the buffer
zone per week.}
\end{figure}
\medskip{}
...
\begin{figure}[H]
\begin{centering}
\includegraphics[width=1\textwidth]{figures/newFig/FigS9}\hspace{2mm}
\par\end{centering}
...
Table \ref{tab:SafetyScenarios}).}
\end{figure}
\medskip{}

\begin{figure}[H]
\begin{centering}
\includegraphics[width=1\textwidth]{figures/newFig/FigS10}\hspace{2mm}
\par\end{centering}
...
size of the camp with no interventions, and with a safety zone with
2 contacts per week.}
\end{figure}
...
\begin{figure}[H]
\begin{centering}
\includegraphics[width=1\textwidth]{figures/newFig/FigS11}\hspace{2mm}
\par\end{centering}
...
in the buffer zone from a baseline of 2 per week.}
\end{figure}

\medskip{}

\begin{figure}[H]
\centering{}\includegraphics[width=1\textwidth]{figures/newFig/Fig_Sinterventions}\hspace{2mm}\caption{\label{fig:Suppl_combined} \textbf{Combined interventions.} CFR (top),
and fraction of the population that recovers (bottom) for different
combination of interventions. Evac = evacuation of severely symptomatic,
