...
%DIF DEL Paper_req-550_V1.1.1.tex   Tue Mar 16 12:24:31 2021
%DIF ADD Paper_req-550_V2.0.0.tex   Mon Jul  5 13:29:49 2021
...
\usepackage{xr}
%DIF 32c32
%DIF < \externaldocument{Paper_req-550_SM_V1.1.1}
%DIF -------
\externaldocument{Paper_req-550_SM_V2.0.0} %DIF > 
%DIF -------
\usepackage{lineno}
...
\title{Empowering the crowd: Feasible strategies \\
 \DIFdelbegin \DIFdel{to minimize the spread of COVID-19 }%DIFDELCMD < \\
%DIFDELCMD <  %%%
\DIFdelend \DIFaddbegin \DIFadd{for epidemic management }\DIFaddend in high-density informal settlements\DIFaddbegin \DIFadd{.}\linebreak{}
 \DIFadd{The case of COVID-19 in Northwest Syria}\DIFaddend }
...
\DIFdelbegin \DIFdel{similar }\DIFdelend reduction in mortality \DIFaddbegin \DIFadd{by up to 18\%}\DIFaddend can be achieved by providing 1
self-isolation tent per \DIFdelbegin \DIFdel{200 }\DIFdelend \DIFaddbegin \DIFadd{8 }\DIFaddend people. Protecting the most vulnerable in
...
mortality by \DIFdelbegin \DIFdel{almost 80}\DIFdelend \DIFaddbegin \DIFadd{morethan 90}\DIFaddend \% and delay an outbreak’s peak by almost
...
\textbf{Word count:} \DIFdelbegin \DIFdel{5738
}\DIFdelend \DIFaddbegin \DIFadd{5898
}\DIFaddend 
...
The \DIFdelbegin \DIFdel{COVID-19 pandemic is intensifying }\DIFdelend \DIFaddbegin \DIFadd{spread of airborne infectious diseases with pandemic potential
}\DIFaddend in regions immersed in protracted armed conflicts\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{GuardianWinter}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend , where large portions
of their populations have become displaced\DIFaddbegin \DIFadd{, is an important challenge
\mbox{%DIFAUXCMD
\cite{san_humanitarian_2020}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . When the displaced population exceeds
...
Urgent action is needed to contain the \DIFdelbegin \DIFdel{virus }\DIFdelend \DIFaddbegin \DIFadd{spread of disease }\DIFaddend in these
settings, a task which necessarily involves the engagement of the
communities living in them \cite{wilkinson_local_2020}.

This study focuses on the \DIFdelbegin \DIFdel{Northwest }\DIFdelend \DIFaddbegin \DIFadd{spread of COVID-19 inNorthwest }\DIFaddend region of
...
to the one presented \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{by Gatto }\emph{\DIFadd{et al.}} \DIFaddend \cite{gatto_spread_2020},
...
conflict regions to understand how to control \DIFaddbegin \DIFadd{diseases like }\DIFaddend COVID-19
...
a 12-month period inspired by those proposed by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{gatto_spread_2020}
}\hspace{0pt}%DIFAUXCMD
\mbox{%DIFAUXCMD
\cite{bertuzzo_geography_2020} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Gatto }\emph{\DIFadd{et al.}}
\DIFadd{and Bertuzzo }\emph{\DIFadd{et al.}} \DIFadd{\mbox{%DIFAUXCMD
\cite{gatto_spread_2020,bertuzzo_geography_2020}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend (see Fig. \ref{fig:Diagram}). The model is adapted to the context
...
which is simulated following the \DIFdelbegin \DIFdel{tau-leaping approximation }\DIFdelend \DIFaddbegin \DIFadd{Gillespie algorithm }\DIFaddend implemented in
...
The population sizes of informal camps are \DIFdelbegin \DIFdel{approximately log-normally
distributed}\DIFdelend \DIFaddbegin \DIFadd{right-skewed}\DIFaddend , with a mean
of \DIFdelbegin \DIFdel{of }\DIFdelend 1212. We simulated camps with populations of 500, 1000 and 2000
...
of cases suggests that this scenario could \DIFdelbegin \DIFdel{already }\DIFdelend have been reached \cite{UNOCHA_collapse}.
...
\DIFaddbegin \DIFadd{and/or }\DIFaddend adding new population classes that govern behavioural changes
(behaviour-classes)\DIFdelbegin \DIFdel{,
and/or adding a compartment to consider the time until detection of
symptoms (see Fig. \ref{fig:DiagramOnset})}\DIFdelend . Since $\lambda_{i}$ can be factorized in four
...
\begin{figure*}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\textwidth]{/home/apascual/Nextcloud/Militancia/crowdfightcovid19/Projects/Request550-Syria/req-550-Syria/manuscripts/main/figures/Fig1}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\textwidth]{figures/Fig1}
\DIFaddendFL 
...
\hline 
\multirow{8}{0.1\textwidth}{\centering{}Self-isolation} & \multirow{2}{0.17\textwidth}{\centering{}{\small{}$(c_{\care}/c_{i})(\tilde{I}_{j}/N_{i})$}} & \multirow{2}{0.07\textwidth}{\centering{}{\small{}$N/N_{j}$}} & \DIFdelbeginFL %DIFDELCMD < \multirow{8}{0.07\textwidth}{\centering{}{\small{}1}} %%%
\DIFdelendFL \DIFaddbeginFL \multirow{2}{0.07\textwidth}{\centering{}{\small{}1}} \DIFaddendFL & \multirow{8}{0.07\textwidth}{\centering{}{\small{}1}} & \multirow{2}{0.07\textwidth}{\centering{}{\small{}0.2}} & \centering{}{\small{}$i\in$ young healthy adults}\tabularnewline
 &  &  &  &  &  & \centering{}{\small{}$j\in$ isolated individuals in class $j$}\tabularnewline
\DIFdelbeginFL %DIFDELCMD < \cline{2-3} \cline{3-3} %%%
\DIFdelendFL \DIFaddbeginFL \cline{2-4} \cline{3-4} \cline{4-4} \DIFaddendFL \cline{6-7} \cline{7-7} 
 & \multirow{2}{0.17\textwidth}{\centering{}{\small{}1}} & \multirow{2}{0.07\textwidth}{\centering{}{\small{}1}} & \DIFaddbeginFL \multirow{6}{0.07\textwidth}{\mbox{%DIFAUXCMD
$\varTheta(N_{\I}-\tilde{N})$
}%DIFAUXCMD
} \DIFaddendFL &  & \multirow{2}{0.07\textwidth}{\centering{}{\small{}1}} & \centering{}{\small{}$i\in$ young healthy adults}\tabularnewline
...
Materials for their derivations). $c_{\care}=$ number \DIFdelbeginFL \DIFdelFL{ofcontacts
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of contacts
}\DIFaddendFL per day of each isolated individual with carers. \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
$N_{\Exp}=$
}%DIFAUXCMD
potential
carers available (non-symptomatic healthy younger adults). }\DIFdelendFL $\tilde{I}_{j}=$
Number of isolated cases of class $j$. \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
$N_{\I}=$
}%DIFAUXCMD
number of symptomatic
individuals. \mbox{%DIFAUXCMD
$\tilde{N}=$
}%DIFAUXCMD
maximum number of tents available for self-isolation.
\mbox{%DIFAUXCMD
$\Theta(x)=$
}%DIFAUXCMD
Heaviside function. }\DIFaddendFL $c_{\family}=$ maximum number of
...
the camp.
\DIFdelbegin \DIFdel{We also considered minimum time intervals for individuals
...
an additional compartment (\mbox{%DIFAUXCMD
$O_{i}$
}%DIFAUXCMD
) between the presymptomatic \mbox{%DIFAUXCMD
$(P_{i}$
}%DIFAUXCMD
)
and clinical symptomatic (\mbox{%DIFAUXCMD
$I_{i}$
}%DIFAUXCMD
) compartments (see Suppl. Fig.
...
\ref{tab:tramsParams} and \ref{tab:TableSeverity}. \DIFaddbegin \DIFadd{We note that
...
distribution (to \mbox{%DIFAUXCMD
$1/\kappa$
}%DIFAUXCMD
in the case of the symptomatic compartment).
...
\DIFaddend The Next Generation Matrix is also computed at each integration step
...
200 people), self-isolation yields a \DIFdelbegin \DIFdel{modest }\DIFdelend \DIFaddbegin \DIFadd{marked }\DIFaddend decrease in the probability
of \DIFdelbegin \DIFdel{observing an outbreak (}\DIFdelend \DIFaddbegin \DIFadd{outbreak (\mbox{%DIFAUXCMD
$\sim$
}%DIFAUXCMD
26\%) (}\DIFaddend see Fig. \ref{fig:Panel2}D) \DIFdelbegin \DIFdel{, but a stronger
reduction in mortality (\mbox{%DIFAUXCMD
$\sim$
}%DIFAUXCMD
30\%) (}\DIFdelend \DIFaddbegin \DIFadd{but there
is no significant reduction in the mortality (}\DIFaddend see Fig. \ref{fig:Panel2}E)\DIFdelbegin \DIFdel{.
Further increasing }\DIFdelend \DIFaddbegin \DIFadd{,
...
It is needed to further increase }\DIFaddend the number of tents past this level
\DIFdelbegin \DIFdel{yields diminishing
returns to reductions to both probability of outbreak (until there
is 1 tent for every 20 people) and mortality (until there is }\DIFdelend \DIFaddbegin \DIFadd{up to at least }\DIFaddend 1 tent for every 8 people \DIFdelbegin \DIFdel{, Conover-Iman test, p-val\mbox{%DIFAUXCMD
$<10^{-16}$
}%DIFAUXCMD
). Similarly,
we }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend observe a reduction \DIFdelbegin \DIFdel{in IFR when at least 10 isolation tents are
set up (see Supplementary Fig. \ref{fig:Suppl_isolation}). Importantly,
the potential reductions in }\DIFdelend \DIFaddbegin \DIFadd{of
\mbox{%DIFAUXCMD
$\sim$
}%DIFAUXCMD
18\% in the mortality and an increase of \mbox{%DIFAUXCMD
$\sim$
}%DIFAUXCMD
16\% in the
time to peak of symptomatic individuals. Increasing the number of
tents does not further reduce the }\DIFaddend probability of outbreak\DIFdelbegin \DIFdel{, overall fatalities
...
to 48h (see Supplementary Fig. \ref{fig:Suppl_onset}}\DIFdelend \DIFaddbegin \DIFadd{. There is
...
Fig. \ref{fig:Suppl_isolation}}\DIFaddend ).
...
zone \DIFaddbegin \DIFadd{(which decreases by 55\%) }\DIFaddend and overall mortality \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend which decreases
by 20\%\DIFaddbegin \DIFadd{) }\DIFaddend when the population is reduced from 2000 to 500. However,
...
cases, mortality is reduced by $\sim$\DIFdelbegin \DIFdel{80}\DIFdelend \DIFaddbegin \DIFadd{94}\DIFaddend \% and the \DIFdelbegin \DIFdel{time }\DIFdelend \DIFaddbegin \DIFadd{probability of
outbreak in the green zone is very small ( \mbox{%DIFAUXCMD
$\sim$
}%DIFAUXCMD
0.005). In other
...
50\%) thetime }\DIFaddend to peak of symptomatic cases in the green zone is delayed
by \DIFdelbegin \DIFdel{two months.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{48 days
...
tents, as low as 5-10 tents per 1000 camp residents \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{in preventing
an outbreak in the camp, but it requires at least 125 tents per 1000
camp residents to substantially reduce mortality }\DIFaddend After conversations
...
\DIFaddbegin \DIFadd{We note that in our simulated intervention individuals become isolated
...
of individuals)\DIFaddbegin \DIFadd{, or reproducing empirically-observed residence times}\DIFaddend .
An example might be the isolation of an individual and his/her family,
as proposed \DIFdelbegin \DIFdel{in Gilman }\DIFdelend \DIFaddbegin \DIFadd{by Gilman }\emph{\DIFadd{et al.}} \DIFaddend \cite{gilman_IBMrefugee_covid20}
