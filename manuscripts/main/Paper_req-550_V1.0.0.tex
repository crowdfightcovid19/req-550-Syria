...
\usepackage{lipsum}
\usepackage{xr}
\externaldocument{Paper_req-550_SM_V1.0.0}
\usepackage{lineno}
...
settings by explicitly parametrizing the camps’ demographics, living
...
We considered a  model, simulating a viral outbreak in a single camp
over a 12-month period. The model, adapted for NWS IDP camps is divided
into compartments containing individuals at different possible stages
along the disease's progression \cite{gatto_spread_2020} {[}add Bertuzzi{]}
(see Fig. \ref{fig:Diagram}). The model then splits the population
into classes to account for the fact that different subgroups of the
population require specific parameters. If there are no interventions,
the distinction in classes dependend only onage and comorbidity status
(hereafter ``demographic-classes''). Some interventions may further
split a demographic-class into subclasses having different behaviour
(``behaviour-classes''). Therefore, different interventions may
require models with different number of classes. We refer to demographic-
and behaviour-classes generically as ``classes'' and we detail the
implementation of behaviour-classes in section XXX.Working with population
...
We consider a model inspired by the one proposed in \cite{gatto_spread_2020},governed
...
\end{gather}
\\
where $i$ indexes the different population classes. The susceptible
population ($S_{i}$) becomes exposed (at rate $\lambda_{i}$), with
exposed individuals ($E_{i}$) getting (at rate $\delta_{\EE}$) into

  a preclinical infectious stage ($P_{i}$) until they reach (at rate
$\delta_{\PP}$) either a clinical (symptomatic, $I_{i}$, with probablity
$f$) or subclinical (asymptomatic, $A_{i}$, with probability $1-f$)
infectious stage. Asymptomatic cases recovered ($R_{i}$) at rate
$\gamma_{\A}$. Clinical cases exit the compartment becoming severe
($H_{i}$) at rate $\eta$, recovering (at rate $\gamma_{\I}$) or
 dying ($D_{i}$) at rate $\alpha$. The demographic-class-specific
parameters $h_{i}$, $g_{i}$ and $l_{i}$ are adjusted to guarantee
that the proportions of clinical symptomatic cases resolving through
each scenario are consistent with the literature (see section severity
assumptions). Finally, individuals at the hospitalized compartment
may either (with probability $\sigma$ at rate $\gamma_{\HH}$) or
recover (at the same rate with probability $1-\sigma$). Behaviour-classes
parameters are encoded in $\lambda_{i}$, as detailed in the next
section.  
...
\begin{gather}
\lambda_{i}=\sum_{j=1}^{n}\tau C_{ij}\frac{\beta_{\PP}P_{j}+\beta_{\text{\A}}A_{j}+\beta_{\I}I_{j}+\beta_{\HH}H_{j}}{N_{j}},\label{eq:lambda-1}
\end{gather}
\\
...
is the total population size of class $j$. We parametrized $C_{ij}$
estimating from conversations with camp managers in NWS the mean number
of contacts that individuals from a population class $i$ have per
day, $c_{i}$, and multiplying this quantity by the probability of
random interaction with other individuals of class $j$. Considering
a well-mixed population, this probability is proportional to class'
$j$ fraction of the population, i.e. $C_{ij}=c_{i}N_{j}/N$. If interventions
are absent, we consider demographic-classes only and, hence, different
$c_{i}$ reflect a different number of contacts due to different ages
and comorbidity status.

The probability of infection if there is a contact between a susceptible
and an infected person for each stage are $\tau\beta_{\PP}$, $\tau\beta_{\A}$,
$\tau\beta_{\I}$ and $\tau\beta_{\HH}$. The $\tau$ parameter is
the maximum transmissivity, which is observed at the presymptomatic
stage {[}Ref{]}. Thus, we selected the presymptomatic stage as a reference
($\beta_{P}=1$) with the remainder $\beta$ parameters estimated
from literature relative to $\beta_{\PP}$ ($\beta_{i}<\beta_{\PP},\quad i\in\{\A,\HH,\I\}$,
see Table XX). The $\tau$ parameter was estimated by randomly generating
a value for the basic reproduction number, $R_{0}$, following a Gaussian
distribution with mean of 4 (99\% CI: 3--5) and dividing this value
by the real part of the main eigenvalue of the Next Generation Matrix
(see section XXX for details on the computational implementation and
Supplementary Material for the analytical results). The distribution
of $R_{0}$ was a compromise between values reported in the literature
from regions with high-density informal settlements: $R_{0}$=2.77
in Abuja and 3.44 in Lagos, Nigeria \cite{oyinlola_empirical_2020},
3.3 in Buenos Aires \cite{santos_numerical_2020}, and 5 in Rohingya
refugee camps in Bangladesh \cite{truelove_potential_2020}.
...
and individuals with comorbidities. Full details of the methods to
estimate the fraction of a simulated camp's population in each of
these 5 classes are reported in Supplementary Material, and final
values shown in Supplementary Table \ref{tab:PopParams}.
...
not have access to official data on healthcare occupancy, the currently
...
critical ($q_{i}^{\DD}$)and recover ($q_{i}^{\RR}$) are demographic-class-specific
(see Table XXX) We estimated these parameters using data from developed
countries with superior population health \cite{dong2020epidemiological,covid2020preliminary}.
...
of 10 years older age groups in developed countries. Since the rates
at which clinical symptomatic individuals ($I_{i}$) resolve into
these three epidemiological outcomes is different ($\eta$ for $H$,
$\alpha$ for $D$ and $\gamma_{\I}$ for $R$) we introduced three
parameters, $h_{i}$, $g_{i}$ and $l_{i}$, to distribute individuals
according to the desired proportions:

\begin{eqnarray*}
q_{i}^{\HH} & = & h_{i}\eta z^{-1}\\
q_{i}^{\DD} & = & g_{i}\alpha z\\
q_{i}^{\RR}= & 1-q_{i}^{\HH}-q_{i}^{\DD} & =l_{i}\gamma_{\I}z^{-1},
\end{eqnarray*}

where $z=h_{i}\eta+g_{i}\alpha+l_{i}\gamma_{\I}.$ The solution ...
XXX in Suppl. Material.
...
We considered a continuos-time Markov process with discrete variables,
in which individuals jump from one epidemiological compartment to
another with instant transition rates determined by Eqs. \ref{eq:System1}--\ref{eq:System8}.
The specific values of the parameters shown in these equations and
of $R_{0}$ are independently drawn at each integration step from
the probability distributions shown in Table XXX. The Next Generation
Matrix is also computed at each integration step from the parameters
drawn, and $\tau$ is then estimated. To ensure computational efficiency,
all these random values are precalculated. In the code provided it
is possible to fix the seed to exactly reproduce the results presented.
The equations are then integrated with a Gillespie algorithm implented
in the adaptivetau package {[}Ref{]} in R {[}Ref{]}. Our simulations
start with a completely susceptible population where one person in
the younger adult population is exposed to the virus. We verified
that a steady state was always reached before the end of each simulation.
We did not consider migration, births, nor deaths due to other causes,
since they are small enough in magnitude to not significantly impact
the course of an outbreak, provided additional conflict does not erupt.
...
$i$ have in a camp per day, $\bar{c}_{i}^{0}$ (see Table \ref{tab:PopParams}),
...
We considered as a null model (indexed with superscript 0) a well-mixed
population in which, if there are no interventions, the average number
of contacts that individuals of class $i$ have in a camp per day
is $\bar{c}_{i}^{0}$ (see Table XX), and hence the null contacts
matrix is:
...
Some of the interventions we considered either reduce the average
number of contacts of class $i$ (e.g. self-isolation) by a factor
$\epsilon_{i}$ or the probability that individuals of class $i$
interact with those of class $j$ (e.g. safety zone strategies), by
a factor $m_{ij}$. The contact matrix resulting from management strategies
can therefore be written with respect to the null model as:

\begin{equation}
C_{ij}=\epsilon_{i}m_{ij}\bar{c}_{i}^{0}N_{j}/N=\epsilon_{i}m_{ij}C_{ij}^{0}=M_{ij}C_{ij}^{0}.\label{eq:ManagementMatrix}
\end{equation}

We name the matrix $M_{ij}$ the management matrix. In addition, in
some specific interventions, we envisioned the existence of zones
(called buffer zones) where the use of masks is mandatory, reducing
the transmissivity by 80\%, i.e. $\hat{\tau}=0.8\tau$.
...
contact rate $c_{i}^{0}$ (see  Table XXX) is  was reduced a fraction
$\epsilon_{i}=\epsilon$ (see XXX).. Since the mean number of inhabitants
...
of 12, 24 and 48 hours (see Fig. \ref{fig:Interventions}-2b), which
was implemented incorporating an additional compartment ($O_{i}$)
between the presymptomatic $(P_{i}$) and clinical symptomatic ($I_{i}$)
compartments The derivantion of $C_{ij}$ for this intervention is
...
the contact rate with individuals living in a different zone, implies
an increase in the contact rate with individuals in the same zone
(see Supplementary Material). Although we do not expect this assumption
to be true in general, it allows us to investigate undesired side-effects
of this intervention, such as older adults having increased contacts
between them if isolated together. Since proposals for partitioning
...
(see Fig. \ref{fig:Interventions}-3). In some of these scenarios,
implementing this intervention requires the split of some demographic-classes
into behaviour-classes. For example, if some healthy younger adults
are allocated into the green zone, we split the demographic-class
``healthy younger adults'' into ``orange'' and ``green'' behaviour-classes,
to model the different contacts that these two subclasses of healthy
younger adults will have between them and with other classes. In Supplementary
Table \ref{tab:SafetyScenarios} we present the classes considered
in each scenario, and the explicit derivation of $C_{ij}$ is presented
...
the package PMCMRplus \cite{pohlert_pmcmr_2020}. 
...
for useful discussions. We thank two anonymous reviewers...
