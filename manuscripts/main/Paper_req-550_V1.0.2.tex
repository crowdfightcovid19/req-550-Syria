...
it is not required the consideration of additional behavioural-classes.
Instead, the intervention can be encoded in $\lambda_{i}$ by simply
splitting the contribution of symptomatic individuals to the rate
of exposure in two terms, one for the isolated individuals and another
one for the remainder. Hence, its implementations requires estimatingthe
parameters $\epsilon_{ij}$ and $\omega_{ij}$  for isolated and non-isolated
individuals individually, and it is also dependent on whether the
interaction is with the younger adult class with no comorbidities
(from which carers are drawn) or with the other classes. In Table
\ref{tab:IntervParams} we present these terms and its derivation
