...
\usepackage{lipsum}
\usepackage{xr}
\externaldocument{Paper_req-550_SM_V1.0.2}
\usepackage{lineno}
...
at the presymptomatic stage for those individuals becoming symptomatic
\cite{he2020temporal}. Thus, we selected these  individuals as a
reference ($\beta_{\PP\rightarrow\I}=1$) with the remaining $\beta$
parameters set relative to $\beta_{\PP\rightarrow\I}$ ($\beta_{i}<\beta_{\PP\rightarrow\I},\quad i\in\{\P,\A,\HH,\I\}$)
(see Table \ref{tab:tramsParams}, Supplementary Materials for derivation).
In particular, the overall presymptomatic transmissibility ($\beta_{\P}$)
is estimated as a weighted average of the transmissibility of individuals
becoming symptomatic and those asymptomatic, considering that the
relative transmissibility between both groups is the same than between
symptomatic and asymptomatic (see Supplementary Materials for details).
...
relative to $\tau$ & 0.93 (95\% CI: 0.88-0.99) & Empirical\_ & Derived\tabularnewline
\hline 
...
symptoms (see Fig. \ref{BROKEN: Ref: fig:DiagramOnset}). Since $\lambda_{i}$
...
Materials for their derivations). $c_{\care}=$ mean contacts per
day of carers with isolated individual. $N_{\Exp}=$ potential carers
available (non-symptomatic healthy younger adults). $\tilde{I}_{j}=$
...
$\zeta_{\HH}$ are set to 0 health checks to access to the buffer
...
in compartments $\I$ or $\HH$ ) self-isolating in individual tents
in dedicated parts of the camps. We excluded from the intervention
individuals with severe symptoms ($\HH$ compartment) since they require
additional care not compatible with self-isolation. Instead, we considered
as an additional intervention the possibility of evacuating these
individuals from the camp (see below). We simulated self-isolation
with various numbers of isolation tents per camp, ranging from 10
to 2000 for a camp of 2000 people (see Fig. \ref{fig:Interventions}-2).
...
are drawn from the younger adults class with no comorbidities who
are not at a stage of the disease during which they display symptoms
(i.e. not in compartments $\I$ or $\HH$ ), while isolated individuals
may belong to any class except children (0-12 yrs.), for whom we do
not consider self-isolation realistic. Since this intervention only
modifies the infectiousness of individuals at the $\I$ compartment
and the exposure of carers, it is not required the consideration of
additional behavioural-classes. Instead, the intervention can be encoded
in $\lambda_{i}$ by simply splitting the contribution of symptomatic
individuals to the rate of exposure in two terms, one for the isolated
individuals and another one for the remainder. Hence, its implementations
requires estimatingthe parameters $\epsilon_{ij}$ and $\omega_{ij}$
 for isolated and non-isolated individuals individually, and it is
also dependent on whether the interaction is with the younger adult
class with no comorbidities (from which carers are drawn) or with
the other classes. In Table \ref{tab:IntervParams} we present these
terms and its derivation in Supplementary Materials.
...
from entering the buffer zone (see Fig. \ref{fig:Interventions}-3b)
...
Although overall contact rates are conserved in this intervention,
we modify the contact rates between $i$ and $j$ with $\epsilon_{ij}$
and the probability of interaction between $i$ and $j$ with $\omega_{ij}$,
where both parameters are determined by whether $i$ and $j$ are
in the same or different zones (see Table \ref{tab:IntervParams}
for specific values).
...
To implement this model, we considered a continuous-time Markov process
with discrete variables, in which individuals jump from one epidemiological
compartment to another with instant transition rates determined by
Eqs. \ref{eq:System1}--\ref{eq:System8}. The specific values of
the parameters shown in these equations and of $R_{0}$ are independently
drawn at each integration step from the probability distributions
shown in Tables \ref{tab:FixedParams}, \ref{tab:tramsParams} and
\ref{tab:TableSeverity}. The Next Generation Matrix is also computed
at each integration step from the parameters drawn, and $\tau$ is
then estimated. To ensure computational efficiency, all these random
values are precalculated. In the code provided it is possible to fix
the seed to exactly reproduce the results presented. The equations
are then integrated with a Gillespie algorithm implemented in the
adaptivetau package \cite{R_adaptivetau} Our simulations start with
a completely susceptible population where one person in the younger
adult population is exposed to the virus (who is also in the orange
zone if the safety zone intervention is in place). We verified that
a steady state was always reached before the end of each simulation.
...
peaks, as well as the infection fatality rate (IFR), the fractions
of the population that are recovered and remain susceptible at steady-state.
For consistency, we only considered simulations in which there was
an outbreak when comparing the outcome of a variable between interventions.
We used the Shapiro-Wilk test \cite{shapiro_analysis_1965} to verify
that our results do not exhibit normally distributed residuals, Kruskal-Wallis
test for pairwise comparisons \cite{kruskal_ranks_1952}, and Conover-Iman
...
the probability of observing an outbreak is close to 0.86, in which
$\apprge$10\% of the camp dies, the number of symptomatic cases peaks
after 37 days, $\sim$84\% of the population recovers, and $\sim$5\%
...
the probability of an outbreak which decreases linearly with the decrease
in percentage reduction (slope).  (see Fig. \ref{fig:Panel2}A). A
near-quadratic trend is observed for the fraction of population dying,
with a faster decrease for large reduction in daily contacts, achieving
a 35\% reduction in mortality.  when the number of contacts is reduced
by 50\% (see Fig. \ref{fig:Panel2}B). Self-distancing also significantly
extends the time until the peak of the outbreak, from \textasciitilde 37
days when there is no intervention to 71 days when contacts are reduced
by 50\% (see Fig. \ref{fig:Panel2}C).  The proportion of population
escaping from infection after 12 months also increases up to near
25\% of the population when a 50\% reduction is considered, and the
proportion of the population recovered after 12 months (which may
help to understand how far the population is from herd immunity) is
...
Further increasing the number of tents does not significantly reduce
the mortality  (Kruskal-Conover post-hoc-test, p-val$<3\times10^{-5}$)
and increases the probability of an outbreak. This counter-intuitive
effect is a consequence of demanding having one independent carer
per eachindividual isolated (see Supplementary Material), so when
the isolated (infected) population increases, the number of healthy
younger adults in contact with them increases in tandem, effectively
hindering the benefits of isolation. This assumption allow us to illustrate
how increasing the isolation capacity is not benefitial unless it
goes hand-in-hand with an appropriate management of the population
isolated (see Discussion).Similarly, we observe a reduction in IFR
when the number of tents is 1 per every 200 people,  and no significant
reduction over that number (see Supplementary Fig. \ref{fig:Suppl_isolation}).
Importantly, the potential reductions in overall fatalities and IFR
...
for the protected population, by around 10\%, if only two contacts
