...
\usepackage{xr}
\externaldocument{Paper_req-550_V0.6}
\usepackage{lineno}
\usepackage{xcolor}
\usepackage{tikz}
...
of class $i$ with an individual of class $j$ per day, as proposed
in Gatto \textit{et al}. \cite{gatto_spread_2020}. $N_{j}$ is the
total population size of class $j$. The model is illustrated in Fig.
\ref{fig:Diagram}, and the rest of the parameters are described in
Tables \ref{tab:PopParams} and \ref{tab:FixedParams}. The compartments
describe the progression of individuals through the different stages
of the disease, starting from susceptible individuals (S), that may
become exposed (E), and then we considered as infectious stages a
presymptomatic stage (P), followed by either an asymptomatic (A) or
symptomatic (I) stage, and severe cases having a longer infectious
period (H). Finally, all cases either recover (R) or die (D). As it
was explained in the Main Text, due to the limited health-care resources
on the Northwest region of Syria (NWS), we considered that individuals
will not have access to hospitals. As a consequence, all cases requiring
ICU would die. It is more difficult to estimate the fate of those
cases requiring hospitalization but not ICU. To address this point,
the compartment H accounts for this cases considering that these individuals
will be infectious for a longer period, and then we considered two
limiting cases for their possible fate: either all individuals in
the H compartment will die $\sigma=1$, or all will recover $\sigma=0$.
Simulating both possibilities allow us to to estimate upper and lower
bounds for the outcome variables of our model.
