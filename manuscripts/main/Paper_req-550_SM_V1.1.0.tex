...
\usepackage{xr}
\externaldocument{Paper_req-550_V1.1.0}
\usepackage{lineno}
\usepackage{xcolor}
\usepackage{tikz}
...
stage for those individuals who become symptomaticas a reference ($\beta_{\PP\rightarrow\I}=1)$,
...
of infectivity in individuals who go on to develop symptoms that occurs
at the presymptomatic stage ($\X\in\{\PP\}$) as the area under the
infectivity curve prior to symptom onset, $AUC_{\PP}$, and the proportion
of infectivity that occurs at symptomatic stages ($\X\in\{\I,\HH\}$)
as the area under the infectivity curve after symptom onset, $1-AUC_{\PP}$
\cite{he_temporal_2020}:
...
in Main Text), the rate of exposure becomes
...
carers, and each isolated individual requires $c_{\care}$ contacts
per day with isolated individuals, and each class $j$ has $\tilde{I}_{j}$
isolated individuals, the mean number of contacts that each carer
has per day with individuals in isolation is $\tilde{c}_{k}=c_{\care}\sum_{j}\tilde{I}_{j}/$$N_{\textrm{care}}$.
In our simulations, each isolated individual receives one visit perday,
i.e. $c_{\care}=1$. We envisage these interactions occuring in what
we call buffer zones, namely open spaces in which carers and isolated
individuals maintain a distance and wear masks, reducing the transmissibility
by 80\% ($\xi=0.2$). Given these assumptions, a general expression
for the rate of exposure of younger adults with no comorbidities,
$\lambda_{k}$, is given by
...
\begin{figure}[H]
\centering{}\includegraphics[height=0.7\textheight]{figures/FigS13}\hspace{2mm}\caption{\label{fig:Suppl_combined} \textbf{Combined interventions.} IFR (top),
...
along the simulation, in the number of individuals simultaneously
exposed, and it increases the time to peak of the infection. When
the reduction goes beyond 50\%, there is an abrupt drop in the time
to peak and in the fraction of population dying, which suggests there
is a critical number of individuals exposed under which the spread
of the virus becomes damped. In other words, above this threshold,
the virus remains in the population a longer time, increasing mortality.
(Right) Fraction of the simulations which do not achieve the critical
number of exposed individuals (that we set \textless 2\%). The abrupt
transition between 50 and 70\% of reduction in the number of contacts
is apparent.}
