...
strategies \\ to minimize the spread of of airborne infectious diseases
...
i.e. $R_{0}=|\lambda_{1}|$, and $\tau$ is estimated from the real
dominant eigenvalue of $\tilde{K}_{\mathrm{S}}=K_{\mathrm{S}}/\tau$.
Considering the null model parameters ($\tilde{\lambda}_{1}^{0}$),
...
zero (see parameters $\zeta_{\I}$ and $\zeta_{\HH}$ in Eq. \ref{eq:lambdaSelf1}
...
become exposed (see Eq. \ref{eq:lambdaSelf1} in Main Text).
...
in the parameter $\omega_{ij}$ (see Eq. \ref{eq:lambdaSelf1} in
...
\dot{P}_{i}=\delta_{\EE}E_{i}-\delta_{\PP}P_{i}\\
\dot{A}_{i}=(1-f)\delta_{\PP}P_{i}-\gamma_{\A}A_{i}\\
\dot{O}_{i}=f\delta_{\PP}P_{i}-\left(\psi_{i}(l_{i}\gamma_{I}+h_{i}\eta+g_{i}\alpha)+(1-\psi_{i})\delta_{\OO}\right)O_{i}\\
\dot{I}_{i}=(1-\psi_{i})\delta_{\OO}O_{i}-(l_{i}\gamma'_{I}+h_{i}\eta'+g_{i}\alpha')I_{i}\\
\dot{H}_{i}=h_{i}(\eta'I_{i}+\psi_{i}\eta O_{i})-\gamma_{\HH}H_{i}\\
\dot{R}_{i}=\gamma_{\A}A_{i}+l_{i}(\gamma'_{\I}I_{i}+\psi_{i}\gamma_{\I}O_{i})+(1-\sigma)\gamma_{\HH}H_{i}\\
\dot{D_{i}}=g_{i}(\alpha'I_{i}+\psi_{i}\alpha O_{i})+\sigma\gamma_{\HH}H_{i}\label{eq:System8}
\end{gather}

where the duration of $O_{i}$ follows a Gaussian distribution with
means $1/\delta_{\OO}\in$\{12, 24, 48\} hours. In the symptomatic
compartment prior to identification, $O_{i}$, individuals may die,
recover or progress towards hospitalization at rates $\alpha$, $\gamma_{\I}$
and $\eta$, respectively --i.e at the rates set for symptomatic
individuals when there is no self-isolation. Their proportion depends
on the time spent in the compartment. At a given time $t=t_{0}$ the
population in the compartment decay is lower bounded by $O_{i}(t)=O_{i}^{0}\exp(-h_{i}t)$
with $h_{i}=(l_{i}\gamma_{I}+h_{i}\eta+g_{i}\alpha+\delta_{\OO})O_{i}^{0}$
and $O_{i}^{0}=O_{i}(t=t_{0})$. We estimate the proportion of individuals
progressing to the R, D and H compartments during a time period $1/\delta_{\OO}$
as $\psi_{i}=\left(\exp(-O_{i}^{0})-\exp(-h_{i}/\delta_{\OO})\right)/\left(1-\exp(-h_{i}/\delta_{\OO})\right)$
where $O_{i}^{0}$ is the value of $O_{i}$ at the integration step
considered. Therefore, a proportion $1-\psi$ progresses to the clinical
symptomatic compartment. The times spent by individuals in the clinical
symptomatic compartment progressing towards death ($1/\alpha'$),
recovery ($1/\gamma_{I}'$) or hospitalization ($1/\eta$), are calculated
as the difference between the symptomatic period if there is no isolation,
($1/\alpha'$ $1/\gamma_{I}'$, $\text{1/\ensuremath{\eta}}$) and
the duration spent in the symptom onset compartment ($\text{1/\ensuremath{\delta}}_{\OO}$).
The relative transmissibility in the symptomatic compartment prior
to identification, $O_{i}$, remains the same as the one of the symtomatic
compartment, $I_{i}$ ($\beta_{I}$).
...
\begin{tikzpicture}[node distance=2.5cm,auto,>=latex']     
    ...
    \node (O) [int, below of=P] {$\dot{O_i}$};
	\node (blank) [right of=O, coordinate]{}; 
	\node (I) [int, right of=blank] {$\dot{I_i}$}; 
	\node (A) [int, above of=P] {$\dot{A_i}$}; 
    ...
    \path[->] (O) edge node [anchor=center, above, midway] {$(1-\psi) \delta_\OO O_i$} (I);
	\path[->] (O) edge node [anchor=center, left, pos=0.65] {$\psi \l_i \gamma_\I O_i$} (R);
	\path[->] (O) edge node [anchor=center, left, pos=0.75] {$\psi g_i{\alpha}O_i$} (D);
	\path[->] (O) edge[bend right] node [anchor=center, above, pos=0.6] {$\psi h_i{\eta}O_i$} (H);   
	\path[->] (I) edge node [anchor=center, above, midway] {$h_i{\eta'}I_i$} (H);   
	\path[->] (I) edge node [anchor=center, right, pos=0.5] {$\l_i \gamma'_\I I_i$} (R);   
...
To implement self-isolation it is required a delineation between the
isolated and non-isolated population within the clinical symptomatic
compartment after identification of symptoms ($I_{i}$). This does
not entail the creation of a new compartment, but rather the estimation
of the number of individuals in each class in isolation ($\tilde{I}_{i}$).
...
with no comorbidities, hereafter indexed $k$. The general expression
of $\lambda_{k}$ presented in the Main Text is:

\medskip{}

\begin{equation}
\lambda_{k}=\sum_{j=1}^{n}\tau\xi_{kj}c_{k}\epsilon_{kj}\omega_{kj}\frac{N_{j}}{N}\left(\frac{\beta_{\PP}P_{j}+\beta_{\A}A_{j}+\zeta_{\I}\beta_{\I}I_{j}+\zeta_{\HH}\beta_{\HH}H_{j}}{N_{j}}\right).\label{eq:lambdaSelf1}
\end{equation}

\medskip{}

If we have $N_{\care}$ carers, the expression of $\lambda_{k}$ can
be split in two contributions:

\begin{equation}
\lambda_{k}=\frac{N_{\care}}{N_{k}}\lambda_{k}^{\care}+\left(\frac{N_{k}-N_{\care}}{N_{k}}\right)\lambda_{k}^{\overline{\care}}\label{eq:lambdaYoung}
\end{equation}

where the first term in the r.h.s. of the equation is the contribution
from carers, and the second term from the remaining population of
the class. We can proceed similarly now splitting $\lambda_{k}^{\care}$
in two, one term to model the interaction with the $\tilde{I}_{j}$
isolated individuals and another term to account for the interaction
with the $I_{j}-\tilde{I}_{j}$ not-isolated individuals (if the total
number of symptomatic individuals $N_{\I}$ exceeds the capacity of
the camp $\tilde{N}$):

\begin{equation}
\lambda_{k}^{\care}=\tau\sum_{j=1}^{n}\underset{\mathrm{isolated}}{\underbrace{\tilde{\xi}_{kj}\tilde{c}_{k}\overset{\tilde{P}(k\rightarrow j)}{\overbrace{\tilde{\epsilon}_{kj}\tilde{\omega}_{kj}\frac{N_{j}}{N}\frac{\tilde{\zeta}_{\I}\tilde{\beta}_{\I}\tilde{I}_{j}}{N_{j}}}}}}+\underset{\mathrm{not\ isolated}}{\underbrace{\xi_{kj}c_{k}\epsilon_{kj}\omega_{kj}\frac{N_{j}}{N}\left(\frac{\beta_{\PP}P_{j}+\beta_{\A}A_{j}+\zeta_{\I}\beta_{\I}(I_{j}-\tilde{I}_{j})+\zeta_{\HH}\beta_{\HH}H_{j}}{N_{j}}\right)}}.\label{eq:lambdaSelf2}
\end{equation}

where we indicate with tildes those parameters in the isolated population
and we note that, if these parameters are the same than for the not-isolated
population, we retrieve back Eq. \ref{eq:lambdaSelf1}. The split
presented in Eq. \ref{eq:lambdaSelf2} is valid under the well-mixed
assumption. However, this assumption does not hold for the isolated
population, which is confined under very particular conditions. More
specifically, we should re-estimate the term highlighted as $\tilde{P}(k\rightarrow j)$
which is the probability of interaction of the population of carers
with isolated individuals (because, by definition, those fulfilling
their roles as carers will interact with isolated individuals), and
hence $\tilde{P}(k\rightarrow j)=1$.

Therefore, for the isolated term, we should just estimate the coefficients
$\tilde{\xi}_{kj}$ and $\tilde{c}_{k}$. To estimate $\tilde{c}_{k}$
we note that, if the class has $N_{\textrm{care}}$ carers, each isolated
individual requires $c_{\care}$ contacts per day with them, and each
class $j$ has $\tilde{I}_{j}$ isolated individuals, the mean number
of contacts that each carer has per day with individuals in isolation
is $\tilde{c}_{k}=c_{\care}\sum_{j}\tilde{I}_{j}/$$N_{\textrm{care}}$.
In our simulations, each isolated individual receives one visit per
day, i.e. $c_{\care}=1$. Finally, since we envisage these interactions
occuring in what we call buffer zones, namely open spaces in which
carers and isolated individuals maintain a distance and wear masks,
reducing the transmissibility by 80\%, we consider that $\tilde{\xi}_{ij}=0.2$.

For the not-isolated term, we maintain the well-mixed population assumption
explained in the Main Text, with the incorporation of a Heaviside
function, $\zeta_{I}=\varTheta(N_{\I}-\tilde{N})$, which activates
interaction with clinical symptomatic individuals when their number
$N_{\I}$ exceeds the isolation capacity $\tilde{N}$. We should also
readjust the probability of interacting with symptomatic non-isolated
individuals in class $j$ to be proportional to their fraction of
the class' population, $(I_{j}-\tilde{I}_{j})/(N_{j}-\tilde{I}_{j})$.
This yields the following expression for the subpopulation of carers:

\begin{equation}
\lambda_{k}^{\care}=\tau\sum_{j}\underset{\mathrm{isolated}}{\underbrace{\xi_{ij}\beta_{\I}\tilde{c}_{k}}}+\underset{\mathrm{not\ isolated}}{\underbrace{c_{k}\left(\frac{N_{j}-\tilde{I}_{j}}{N}\right)\left(\frac{\beta_{\PP}P_{j}+\beta_{\A}A_{j}+\beta_{\I}\varTheta(N_{\I}-\tilde{N})(I_{j}-\tilde{I}_{j})+\beta_{\HH}H_{j}}{N_{j}-\tilde{I}_{j}}\right)}}.\label{eq:lambdaCarers}
\end{equation}
...
\begin{equation}
\lambda_{k}^{\overline{\care}}=\underset{\mathrm{not\ isolated}}{\underbrace{c_{k}\left(\frac{N_{j}-\tilde{I}_{j}}{N}\right)\left(\frac{\beta_{\PP}P_{j}+\beta_{\A}A_{j}+\beta_{\I}\varTheta(N_{\I}-\tilde{N})(I_{j}-\tilde{I}_{j})+\beta_{\HH}H_{j}}{N_{j}-\tilde{I}_{j}}\right)}}.\label{eq:lambdaNotCarers}
\end{equation}

Inserting Eqs. \ref{eq:lambdaCarers} and \ref{eq:lambdaNotCarers}
into Eq. \ref{eq:lambdaYoung} yields:

\begin{equation}
\lambda_{k}=\tau\sum_{j}\underset{\mathrm{isolated}}{\underbrace{\xi_{ij}\beta_{\I}c_{\care}\frac{\tilde{I}_{j}}{N_{k}}}}+\underset{\mathrm{not\ isolated}}{\underbrace{c_{k}\left(\frac{\beta_{\PP}P_{j}+\beta_{\A}A_{j}+\beta_{\I}\varTheta(N_{\I}-\tilde{N})(I_{j}-\tilde{I}_{j})+\beta_{\HH}H_{j}}{N}\right)}.}\label{eq:lambdaIsolation}
\end{equation}

We can express this equation following the parameterization presented
in Eq. \ref{eq:lambdaSelf1} making $\tilde{\xi}_{ij}=0.2$, $\tilde{\epsilon}_{ij}=(c_{\care}/c_{i})(\tilde{I}_{j}/N_{i})$,
$\tilde{\omega}_{ij}=N/N_{j}$ and $\tilde{\zeta}_{\I}\tilde{\beta}_{\I}=1$
for $j\in$ isolated individuals in class $j$, while $\xi=1$,$\text{\ensuremath{\epsilon_{ij}=1}, \ensuremath{\omega_{ij}=1}}$
and $\zeta_{\I}=\varTheta(N_{\I}-\tilde{N})$ for $j\in$ individuals
in class $j$ who are not isolated. The rate of exposure of the remaining
classes (not younger adults without comorbidities, $i\neq k$) corresponds
to the second term in the r.h.s of Eq. \ref{eq:lambdaIsolation}:

\begin{equation}
\lambda_{i}=\tau\sum_{j}\underset{\textrm{not isolated}}{\underbrace{c_{i}\frac{\beta_{\PP}P_{j}+\beta_{\A}A_{j}+\beta_{\I}\varTheta(N_{\I}-\tilde{N})(I_{j}-\tilde{I}_{j})+\beta_{\HH}H_{j}}{N}}}.
\end{equation}
...
\begin{figure}[H]
\includegraphics[width=1\textwidth]{figures/FigS5.pdf}\hspace{2mm}\caption{\label{fig:Suppl_onset} \textbf{Time to self-isolation.} Probability
...
of the camp's population is allocated to the safety zone. Note that
...
the population class inhabiting the zone, i.e. the number of simulations
considered to compute each mean may be different. This explains for
example why there is a reduction in the mean time until symptomatic
cases peak when moving from 2 contacts per week without health checks
to considering health checks for the whole population, despite there
being an increase in the safety zone.}
\end{figure}
...
20 cases are recorded. N = 2500 simulations for each combination of
...
\begin{table}[H]
\begin{tabular}{|c|>{\raggedright}m{2.5cm}|c|c|c|c|c|c|c|c|c|}
\hline 
Simul. & \centering{}Experim. & Structure & Npop & Evac. & N.Tents & Contacts & Tcheck & lock & self & H-fate\tabularnewline
\hline 
\hline 
1 & \multirow{6}{2.5cm}{\centering{}A. Shielding and limits} & \multirow{2}{*}{Null Mixed} & \multirow{6}{*}{2000} & \multirow{6}{*}{No} & \multirow{6}{*}{0} & \multirow{2}{*}{MF} & \multirow{2}{*}{No} & \multirow{6}{*}{No} & \multirow{6}{*}{No} & D\tabularnewline
...
6 &  &  &  &  &  &  &  &  &  & R\tabularnewline
\hline 
7 & \multirow{14}{2.5cm}{\centering{}B. Role individual tents} & \multirow{7}{*}{Null Mixed} & \multirow{14}{*}{2000} & \multirow{14}{*}{No} & 10 & \multirow{7}{*}{MF} & \multirow{7}{*}{No} & \multirow{14}{*}{No} & \multirow{14}{*}{No} & \multirow{14}{*}{D}\tabularnewline
\cline{1-1} \cline{6-6} 
8 &  &  &  &  & 20 &  &  &  &  & \tabularnewline
\cline{1-1} \cline{6-6} 
9 &  &  &  &  & 50 &  &  &  &  & \tabularnewline
...
\hline 
21 & \multirow{5}{2.5cm}{\centering{}C. Population shielded} & Shield (age3) & \multirow{5}{*}{2000} & \multirow{5}{*}{No} & \multirow{5}{*}{0} & \multirow{5}{*}{2/7} & \multirow{5}{*}{Yes} & \multirow{5}{*}{No} & \multirow{5}{*}{No} & \multirow{5}{*}{D}\tabularnewline
\cline{1-1} \cline{3-3} 
22 &  & Shield (age2) &  &  &  &  &  &  &  & \tabularnewline
\cline{1-1} \cline{3-3} 
23 &  & Shield (20\%) &  &  &  &  &  &  &  & \tabularnewline
\cline{1-1} \cline{3-3} 
24 &  & Shield (25\%) &  &  &  &  &  &  &  & \tabularnewline
\cline{1-1} \cline{3-3} 
25 &  & Shield (30\%) &  &  &  &  &  &  &  & \tabularnewline
\hline 
26 & \multirow{4}{2.5cm}{\centering{}D. Population size} & \multirow{2}{*}{Null Mixed} & 500 & \multirow{4}{*}{No} & \multirow{4}{*}{0} & \multirow{2}{*}{MF} & \multirow{2}{*}{No} & \multirow{4}{*}{No} & \multirow{4}{*}{No} & \multirow{4}{*}{D}\tabularnewline
\cline{1-1} \cline{4-4} 
27 &  &  & 1000 &  &  &  &  &  &  & \tabularnewline
\cline{1-1} \cline{3-4} \cline{4-4} \cline{7-8} \cline{8-8} 
28 &  & \multirow{2}{*}{Shield (20\%)} & 500 &  &  & \multirow{2}{*}{2/7} & \multirow{2}{*}{Yes} &  &  & \tabularnewline
\cline{1-1} \cline{4-4} 
29 &  &  & 1000 &  &  &  &  &  &  & \tabularnewline
\hline 
30 & \multirow{4}{2.5cm}{\centering{}E. Safety zone testing} & \multirow{4}{*}{Shield (20\%)} & \multirow{4}{*}{2000} & \multirow{4}{*}{No} & \multirow{4}{*}{0} & \multirow{2}{*}{2/7} & No & \multirow{4}{*}{No} & \multirow{4}{*}{No} & \multirow{4}{*}{D}\tabularnewline
\cline{1-1} \cline{8-8} 
31 &  &  &  &  &  &  & Yes &  &  & \tabularnewline
\cline{1-1} \cline{7-8} \cline{8-8} 
32 &  &  &  &  &  & \multirow{2}{*}{10/7} & No &  &  & \tabularnewline
\cline{1-1} \cline{8-8} 
33 &  &  &  &  &  &  & Yes &  &  & \tabularnewline
\hline 
\end{tabular}
...
self-isolation tents per camp. Contacts = Number of contacts per day
between populations shielded.	Tcheck = Are temperature checks performed?	Lock
= Is lockdown applied after first symptomatic case is identified?
self = Fraction of contacts remaining after self-distancing is implemented.
H-fate = Final compartment for hospitalized people. MF = Mean field.
Shield = Population shielded. age3 = elderly population. age2 = adults
...
\hline 
\hline 
34 & \multirow{3}{2.5cm}{\centering{}F. Lockdown} & \multirow{3}{*}{Shield (20\%)} & \multirow{3}{*}{2000} & \multirow{3}{*}{No} & \multirow{3}{*}{0} & \multirow{3}{*}{2/7} & \multirow{3}{*}{Yes} & 0.5 & \multirow{3}{*}{No} & \multirow{3}{*}{D}\tabularnewline
\cline{1-1} \cline{9-9} 
35 &  &  &  &  &  &  &  & 0.9 &  & \tabularnewline
\cline{1-1} \cline{9-9} 
36 &  &  &  &  &  &  &  & 0.99 &  & \tabularnewline
\hline 
37 & \multirow{10}{2.5cm}{\centering{}G. Self-distancing} & \multirow{5}{*}{Null Mixed} & \multirow{10}{*}{2000} & \multirow{10}{*}{No} & \multirow{10}{*}{0} & \multirow{5}{*}{MF} & \multirow{5}{*}{No} & \multirow{10}{*}{No} & 0.1 & \multirow{10}{*}{D}\tabularnewline
\cline{1-1} \cline{10-10} 
38 &  &  &  &  &  &  &  &  & 0.2 & \tabularnewline
\cline{1-1} \cline{10-10} 
39 &  &  &  &  &  &  &  &  & 0.3 & \tabularnewline
\cline{1-1} \cline{10-10} 
40 &  &  &  &  &  &  &  &  & 0.4 & \tabularnewline
\cline{1-1} \cline{10-10} 
41 &  &  &  &  &  &  &  &  & 0.5 & \tabularnewline
\cline{1-1} \cline{3-3} \cline{7-8} \cline{8-8} \cline{10-10} 
42 &  & \multirow{5}{*}{Shield (20\%)} &  &  &  & \multirow{5}{*}{2/7} & \multirow{5}{*}{Yes} &  & 0.1 & \tabularnewline
\cline{1-1} \cline{10-10} 
43 &  &  &  &  &  &  &  &  & 0.2 & \tabularnewline
\cline{1-1} \cline{10-10} 
44 &  &  &  &  &  &  &  &  & 0.3 & \tabularnewline
\cline{1-1} \cline{10-10} 
45 &  &  &  &  &  &  &  &  & 0.4 & \tabularnewline
\cline{1-1} \cline{10-10} 
46 &  &  &  &  &  &  &  &  & 0.5 & \tabularnewline
\hline 
47 & \multirow{2}{2.5cm}{\centering{}H. Evacuation} & Null Mixed & \multirow{2}{*}{2000} & \multirow{2}{*}{Yes} & \multirow{2}{*}{0} & MF & No & \multirow{2}{*}{No} & \multirow{2}{*}{No} & \multirow{2}{*}{D}\tabularnewline
\cline{1-1} \cline{3-3} \cline{7-8} \cline{8-8} 
48 &  & Shield (20\%) &  &  &  & 2/7 & Yes &  &  & \tabularnewline
\hline 
49 & \multirow{19}{2.5cm}{\centering{}I. Combined interventions} & \multirow{2}{*}{Null Mixed} & \multirow{8}{*}{2000} & \multirow{8}{*}{No} & \multirow{8}{*}{50} & \multirow{2}{*}{MF} & \multirow{2}{*}{No} & \multirow{2}{*}{No} & 0.2 & \multirow{8}{*}{D}\tabularnewline
\cline{1-1} \cline{10-10} 
50 &  &  &  &  &  &  &  &  & 0.5 & \tabularnewline
\cline{1-1} \cline{3-3} \cline{7-10} \cline{8-10} \cline{9-10} \cline{10-10} 
51 &  & \multirow{6}{*}{Shield (20\%)} &  &  &  & \multirow{6}{*}{2/7} & \multirow{6}{*}{Yes} & No & 0.2 & \tabularnewline
\cline{1-1} \cline{9-10} \cline{10-10} 
52 &  &  &  &  &  &  &  & 0.5 & No & \tabularnewline
\cline{1-1} \cline{9-10} \cline{10-10} 
53 &  &  &  &  &  &  &  & 0.5 & 0.2 & \tabularnewline
\cline{1-1} \cline{9-10} \cline{10-10} 
54 &  &  &  &  &  &  &  & 0.9 & 0.2 & \tabularnewline
\cline{1-1} \cline{9-10} \cline{10-10} 
55 &  &  &  &  &  &  &  & 0.5 & 0.5 & \tabularnewline
\cline{1-1} \cline{9-10} \cline{10-10} 
56 &  &  &  &  &  &  &  & 0.9 & 0.5 & \tabularnewline
\cline{1-1} \cline{3-11} \cline{4-11} \cline{5-11} \cline{6-11} \cline{7-11} \cline{8-11} \cline{9-11} \cline{10-11} \cline{11-11} 
57 &  & \multirow{2}{*}{Null Mixed} & \multirow{8}{*}{2000} & \multirow{11}{*}{Yes} & \multirow{11}{*}{50} & \multirow{2}{*}{MF} & \multirow{2}{*}{No} & \multirow{2}{*}{No} & 0.2 & \multirow{11}{*}{D}\tabularnewline
\cline{1-1} \cline{10-10} 
58 &  &  &  &  &  &  &  &  & 0.5 & \tabularnewline
\cline{1-1} \cline{3-3} \cline{7-10} \cline{8-10} \cline{9-10} \cline{10-10} 
59 &  & \multirow{9}{*}{Shield (20\%)} &  &  &  & \multirow{9}{*}{2/7} & \multirow{9}{*}{Yes} & No & 0.2 & \tabularnewline
\cline{1-1} \cline{9-10} \cline{10-10} 
60 &  &  &  &  &  &  &  & 0.5 & No & \tabularnewline
\cline{1-1} \cline{9-10} \cline{10-10} 
...
65 &  &  & \multirow{3}{*}{500} &  &  &  &  & No & 0.2 & \tabularnewline
\cline{1-1} \cline{9-10} \cline{10-10} 
66 &  &  &  &  &  &  &  & 0.5 & No & \tabularnewline
\cline{1-1} \cline{9-10} \cline{10-10} 
67 &  &  &  &  &  &  &  & 0.5 & 0.2 & \tabularnewline
\hline 
\end{tabular}
