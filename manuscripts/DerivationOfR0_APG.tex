...
of intervention introducing the parameter $\epsilon$, representing
the fraction of the average number of contacts observed in the null
model that prevail after the intervention: $\bar{c}_{i}=\epsilon\bar{c}_{i}^{0}$.
...
\begin{equation}
C_{ij}=\epsilon m_{ij}\bar{c}_{i}^{0}N_{j}/N=\epsilon m_{ij}C_{ij}^{0}.
\end{equation}
...
\begin{gather}
\lambda_{i}=\frac{\tau}{N}\sum_{j=1}^{n}\bar{c}_{i}\left(P_{j}+A_{j}+I_{j}+H_{j}\right)\label{eq:lambda-1}
\end{gather}
