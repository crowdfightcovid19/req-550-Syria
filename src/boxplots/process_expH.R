...
df$self<-factor(df$self,levels(df$self)[idx.self])
levels(df$self)

fn = "FracFinalDeaths"
varX="self"
varY="fracdeath" 
xlabel="Reduction of personal contact (%)"
...
gg=ggplot(data=df)+
  ...
  geom_point(position=position_jitterdodge(dodge.width=0.9),aes_string(x="self",y=fn,colour="group"))+
  geom_boxplot(aes_string(x="self",y=fn,fill="group"),position=dodge)+
  xlab(xlabel)+
  ylab(ylabel)+
...
pdf(file=filePlotOut,width=9,height = 7)
gg=ggplot(data=df)+
  geom_violin(aes_string(x="self",y=fn,fill="group"))+
...
varY="timepeak" 
xlabel="Reduction of personal contact (%)"
ylabel="Time to peak symptomatic (days)"
titlelabel="Self-distancing"
Npop=2000
