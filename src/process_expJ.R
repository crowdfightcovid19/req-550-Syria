...
levels(df$lock)
idx.lock=c(4,1,2,3) # reorder factors
df$lock=factor(df$lock,levels(df$lock)[idx.lock])
levels(df$lock)
...
gg=ggplot(data=df)+
  ...
  geom_point(position=position_jitterdodge(dodge.width=0.9),aes_string(x="lock",y=fn,colour="group"))+
  geom_boxplot(aes_string(x="lock",y=fn,fill="group"),position=dodge)+
  xlab(xlabel)+
  ylab(ylabel)+
...
pdf(file=filePlotOut,width=9,height = 7)
gg=ggplot(data=df)+
  geom_violin(aes_string(x="lock",y=fn,fill="group"))+
