...
#date = 30th July 2020
#description = Process plots for expJ. TODO: significance
#usage = Run from src/
...
extract_subtable_output_summaries = function(df.out,params.df){
  ...
  for(i in 1:Ncomp){
    ...
    df.tmp <- cbind(df.tmp,rep(intervention.label[i],dim(df.tmp)[1]))
...
gg=ggplot(data=df)+
  ...
  geom_point(position=position_jitterdodge(dodge.width=0.9),aes_string(x="intervention",y=fn,colour="group"))+
  geom_boxplot(aes_string(x="intervention",y=fn,fill="group"),position=dodge)+
  xlab(xlabel)+
  ylab(ylabel)+
...
pdf(file=filePlotOut,width=9,height = 7)
gg=ggplot(data=df)+
  geom_violin(aes_string(x="intervention",y=fn,fill="group"))+
