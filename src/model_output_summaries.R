...
# date = 15th July 2020
# description = This script takes a set of directories generated by the stochastic version of 
...
#    is also estimated the probability of observing an outbreak (i.e. at least one death, a more stringent
#    criteria could be used, but it should be recoded) and the Infection Fatality Ratio.
...
#   used to narrow the statistics down to simulations in which at least one death is observed, and to compute
#   the IFR. The file NumFinalDeaths must be located before NumFinalRecovered.
...
# warning = The fractions of exposed and shielded population are hard-coded, you should modify
#   these numbers if needed.
#
rm(list=ls())
...
get_statistics_frac_total <- function(df,PopSize,Nrealiz,idx){
  ...
  if(!is_empty(idx)){
    ...
    Mean=signif(Mean,digits=4)
    stdErr=signif(stdErr,digits=4)
  }else{
    Mean=NA
    stdErr=NA
  }
  ...
  for(file2proc in files.list.local){
    ...
    if (file2proc == "NumFinalDeaths") {
      ...
      if (is_empty(idx.classS)) {
        ...
        p.outbrk.E = signif(1 - min(frac.nodeath[idx.classE]), digits = 4)
        p.outbrk.S = signif(1 - min(frac.nodeath[idx.classS]), digits = 4)
      ...
      CFR.all=signif(death.totals.all/(death.totals.all+recov.totals.all),digits=4)
      if (is_empty(idx.classS)){
        CFR.E=NA
        CFR.S=NA
      }else{
        CFR.E=signif(death.totals.E/(death.totals.E+recov.totals.E),digits=4)
        CFR.S=signif(death.totals.S/(death.totals.E+recov.totals.S),digits=4)
      }
