...
rates_SEPAIHRD_str_V3 = function(y, parms,t){
  ...
  if(model.type=="stochastic_fixed"){ 
    ...
    gammaA=unlist(parms["gammaA"][[1]])[time]
    ...
    alphaV=unlist(parms["alphaV"][[1]])[time]
  ...
  for(Ref in classes){ # For each population class
    ...
    for(class in classes){ # Consider the probability of contact with own and other pop. classes
      ...
      classV=paste(class,"V",sep=".") # severe
      ...
      yClassH=Tcheck.mat[Ref,class]*(y[classH]+y[classV]) # these individuals do not pass a symptoms' check
    ...
    k=k+1
    dy[k] = g*alpha*y[I] # Infected to V
    k=k+1
    dy[k] = alphaV*y[V] # Infected to V
    k=k+1
