...
rates_SEPAIHRD_str_V2 = function(y, parms,t){
  ...
  Niso = y[inf.idx]
  names(Niso)=names(y)[inf.idx] # in V1 = classes, now we will to sum up below
  ...
  for(Ref in classes){ # For each population class
    ...
    for(class in classes){ # Consider the probability of contact with own and other pop. classes
      ...
      classIR=paste(class,"IR",sep=".") # and infected
      classIH=paste(class,"IH",sep=".") # and infected
      classID=paste(class,"ID",sep=".") # and infected
      ...
      if(isoThr > 0){ # If there are tents for isolation
        ...
        if(Nexp > 0){ # There are carers available 
          idx.tmp=grep(class,names(Niso)) # there are 3 symptomatic compartments, identify
          Niso.class=sum(Niso[idx.tmp]) # sum them up
        ...
        if(Itot > isoThr){ # but if the capacity of isolation is insufficient
          idx.tmp=grep(class,names(Niso))
          Nfree=sum(y[inf.idx[idx.tmp]]-Niso[idx.tmp]) # number of non-isolated exceeding the capacity are infectious
        ...
        idx.tmp=grep(class,y[inf.idx])
        yClassI=Tcheck.mat[Ref,class]*sum(y[idx.tmp]) # all are fully infectious
    ...
    k=k+1
    dy[k] = f*(1-g-h)*deltaP*y[P] # P to IR if 
    k=k+1
    dy[k] = f*h*deltaP*y[P] # P to IH if 
    k=k+1
    dy[k] = f*g*deltaP*y[P] # P to ID if 
    k=k+1
    dy[k] = gammaA*y[A] # Asymptomatic to R
    k=k+1
    dy[k] = gammaI*y[IR] # Infected to R
    k=k+1
    dy[k] = eta*y[IH] # Infected to H
    k=k+1
    dy[k] = alpha*y[ID] # Infected to D
    k=k+1
