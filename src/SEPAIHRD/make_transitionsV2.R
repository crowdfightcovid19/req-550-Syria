...
make_transitions = function(class.names,var.names,hospitalized2){
  ...
  for(Ref in class.names){
    ...
    IR.vars=paste(Ref,"IR",sep=".")
    IH.vars=paste(Ref,"IH",sep=".")
    ID.vars=paste(Ref,"ID",sep=".")
    ...
    A.tmp=cbind(rbind(S.vars,-1,E.vars,+1), # This matrix lists all the transitions for class Ref
            ...
            rbind(P.vars,-1,IR.vars,+1), 
            rbind(P.vars,-1,IH.vars,+1),  
            rbind(P.vars,-1,ID.vars,+1),  
            rbind(A.vars,-1,R.vars,+1), 
            rbind(IR.vars,-1,R.vars,+1),
            rbind(IH.vars,-1,H.vars,+1),   
            rbind(ID.vars,-1,D.vars,+1),   
